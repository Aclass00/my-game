...
function updateMarketDisplay() {
  ...
  if (currentPlayer.marketLevel === 0) {
    marketEl.textContent = '—';
    rateEl.textContent = '0';
    upgradeBtn.textContent = `بناء السوق (تكلفة: ${BUILDINGS.market[0].cost} ذهب)`;
  } else {
...
window.upgradeMarket = function() {
  ...
  if (currentPlayer.gold >= nextMarket.cost) {
    ...
    updateDisplay();
    savePlayerData();
    alert(`✅ تم ${currentPlayer.marketLevel === 1 ? 'بناء' : 'تطوير'} السوق!`);
  } else {
    alert('❌ ذهب غير كافٍ!');
  }
}
